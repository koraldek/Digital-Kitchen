...
//</editor-fold>
    ...
    void initializeUserTest() {
        ...
        when(dietService.initializeDietDay()).thenReturn(dietDay);
        when(dietService.initializeDietDays()).thenReturn(dietDays);
        when(dietService.initializeDiet()).thenReturn(diet);
        when(dietService.initializeDiets()).thenReturn(diets);
        when(socialService.createWelcomeMessage(any())).thenReturn(nMessage);


        User savedUser = userServiceMock
                .postCreateUser(userServiceMock
                        .preCreateUser(usr_Andrzej85));
        assertAll(
