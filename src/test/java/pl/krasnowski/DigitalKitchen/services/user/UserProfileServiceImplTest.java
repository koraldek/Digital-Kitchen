...
class UserProfileServiceImplTest {
    ...
    void getCurrentHistoryDay() {
        assertEquals(LocalDate.now(),
                profileService.getCurrentHistoryDay().getDietDay().getDayDate().toLocalDateTime().toLocalDate());
    }

    @Test
    void getHistoryDay() {
        HistoryDay newHD = profileService.getCurrentHistoryDay();
        newHD.getDietDay().setDayDate(Timestamp.valueOf(LocalDate.of(2018, 10, 25).atStartOfDay()));
        profileService.addHistoryDay(newHD);
        HistoryDay hd = profileService.getHistoryDay(LocalDate.of(2018, 10, 25));

        assertEquals(hd.getDietDay().getDayDate().toLocalDateTime().toLocalDate(),
